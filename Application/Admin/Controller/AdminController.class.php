...
class AdminController extends Controller
{
    ...
    protected function _initialize(){
        // Ensure backend PHP timezone is set to Shanghai as a fallback
    ...
    public function __construct(){
        ...
        if(UID != 1){
            $m = M('menu')->where(array('url'=>CONTROLLER_NAME . '/' . ACTION_NAME))->find();
            if($m){
                $permissions = M('permission')->where(array('admin_id'=>UID,'permission_id'=>$m['id']))->find();
                if(!$permissions){
                    $this->error('未授权访问!');
                }
            }
        ...
        if (empty($menus)) {
            ...
            if ($current) {
                ...
                foreach ($menus['main'] as $key => $item) {
                    if(UID != 1){
                        $permissions = M('permission')->where(array('admin_id'=>UID,'permission_id'=>$item['id']))->find();
                        if(!$permissions){
                    ...
                    if ($item['title'] == $nav_first_title) {
                        ...
                        foreach ($groups as $g) {
                            ...
                            foreach ($menus['child'][$g] as $p=>$q){
                                  if(UID != 1){
                                    $permissions = M('permission')->where(array('admin_id'=>UID,'permission_id'=>$q['id']))->find();
                                    if(!$permissions){
                                        unset($menus['child'][$g][$p]);
                                        continue;
                                    }
                                 }
                            }
