...
class TradeController extends HomeController
{
	...
	{
		parent::_initialize();
		$allow_action=array("index","bborder","bbhistoryorder","tradelist","ordinary","trans","transinfo","upbbbuy","getprice","upbbsell","clearorder","tradebill","billinfo");
	...
	public function index(){
        ...
        $uid = userid();
        $list = M("ctmarket")->where(array('status'=>1))->field("coinname,id")->order('sort asc')->select();
	    ...
	    $coinname = strtoupper($symbol)."/"."USDT";
	    $this->assign('coinname',$coinname);
        $this->assign('symbol',$symbol);
