...
class ContractController extends HomeController
{
	...
	public function creatorder(){
	    if($_POST){
	        ...
	        $bill['num'] = $ctzed;
	        $bill['coinname'] = "usdt";
	        $bill['afternum'] = $minfo['usdt'] - $tmoney;
	        $bill['type'] = 3;
	        $bill['addtime'] = date("Y-m-d H:i:s",time());
	        $bill['st'] = 2;
	        $bill['remark'] = L('购买').$ccoinname.L('秒合约');
	...
	public function index(){	
        ...
        // 确保BNB/USDT排在第一，通过修改数据库排序
        // 获取BNB的当前排序值
        $bnbSort = M("ctmarket")->where(array('coinname'=>'bnb', 'status'=>1))->getField('sort');
        if($bnbSort > 1){
            // 如果BNB的排序值大于1，则将其设置为1，其他币种排序值+1
            M("ctmarket")->where(array('coinname'=>'bnb', 'status'=>1))->save(array('sort'=>1));
            // 将其他币种的排序值都+1，保持相对顺序不变
            M("ctmarket")->where(array('coinname'=>array('neq','bnb'), 'status'=>1))->setInc('sort', 1);
        }
