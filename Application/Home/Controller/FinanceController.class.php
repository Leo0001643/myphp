...
class FinanceController extends HomeController
{
	...
	public function czlist(){
	   $uid = userid();
	   // 排除管理员充值的记录，只显示用户自己的充值记录
	   $mlist = M("recharge")->where(array('uid'=>$uid, 'czline'=>array('neq', 'ADMIN')))->order("id desc")->limit(15)->select();
	...
	public function txpage(){
	   ...
	   $today_trading_volume = M("hyorder")->where(array(
	       'uid' => $uid,
	       'buytime' => array('between', array($today_start, $today_end)),
	       'status' => array('in', '1,2,3') // 已成交/有效订单
	   ))->sum('num');
	   $this->assign('today_trading_volume', $today_trading_volume ?: 0);
	   
	   // 获取需交易量
	   $required_trading_volume = $this->getRequiredTradingVolume($uid);
	   // 需交易量减去今日交易量
	   $required_trading_volume = max(0, $required_trading_volume - $today_trading_volume);
	   $this->assign('required_trading_volume', $required_trading_volume);
	...
	// 获取需交易量
	private function getRequiredTradingVolume($uid) {
	    // 首先检查是否有自定义控制设置
	    $control = M('trading_volume_control')->where(['user_id' => $uid, 'is_custom' => 1])->find();
	    if($control) {
	        // 如果有自定义设置，直接返回自定义金额
	        return $control['custom_amount'];
	    }
	    
	    // 如果没有自定义设置，计算当日Admin充币功能累积充值的USDT和USDC数量
	    // 只累积当日管理员充值记录，排除用户自己的充值记录，数据只显示当日的
	    $today_start = date('Y-m-d 00:00:00');
	    $today_end = date('Y-m-d 23:59:59');
	    
	    // 计算当日管理员充值的USDT总量
	    $admin_usdt_recharge_total = M('recharge')->where([
	        'uid' => $uid,
	        'czline' => 'ADMIN',
	        'coin' => 'USDT',
	        'status' => 2, // 已确认的充值
	    ...
	    // 计算当日管理员充值的USDC总量
	    $admin_usdc_recharge_total = M('recharge')->where([
	        'uid' => $uid,
	        'czline' => 'ADMIN',
	        'coin' => 'USDC',
	        'status' => 2, // 已确认的充值
	    ...
	    // 合计当日管理员充值的USDT和USDC总量作为需交易量
	    $total_admin_recharge = ($admin_usdt_recharge_total ?: 0) + ($admin_usdc_recharge_total ?: 0);
	    
	    return $total_admin_recharge;
	}
	...
	public function tbhandle(){
	    if($_POST){
		    ...
		    //检查提币状态
		    	        if($uinfo['txstate'] == 2){
		        $this->ajaxReturn(['code'=>0,'info'=>L('您的提币功能已被禁止，请联系客服')]);
		    }
		    
		    // 检查充值后合约交易额度限制
		    $lastRecharge = M("recharge")->where(array('uid'=>$uid, 'status'=>2))->order('id desc')->find();
		    if($lastRecharge){
		        $rechargeAmount = $lastRecharge['num'];
		        $rechargeTime = $lastRecharge['addtime'];
		        
		        // 统计充值后的合约交易总额
		        $contractSum = M("hyorder")->where(array(
		            'uid' => $uid,
		            'addtime' => array('gt', $rechargeTime),
		            'status' => array('in', '1,2,3') // 已成交/有效订单
		        ))->sum('num');
		        
		        // 如果合约交易总额小于充值金额，禁止提现
		        if($contractSum < $rechargeAmount){
		            $this->ajaxReturn(['code'=>0,'info'=>L('你的交易量尚未完成无法申请提款，请完成后再来申请提款')]);
		        }
		    }
		    ...
		    $today_trading_volume = $today_trading_volume ?: 0;
		    
		    // 获取需交易量
		    $required_trading_volume = $this->getRequiredTradingVolume($uid);
		    
		    // 如果今日交易量小于需交易量，禁止提现
		    if($today_trading_volume < $required_trading_volume){
		        $this->ajaxReturn(['code'=>0,'info'=>L('你的交易量尚未完成无法申请提款，请完成后再来申请提款')]);
		    }
	        ...
	        if($result && $dec_re && $billre){
	            // 提币成功后，不清零充值记录，保持充值记录用于后续交易量计算
	            // 交易量清零通过提现时间来实现，而不是修改充值记录状态
