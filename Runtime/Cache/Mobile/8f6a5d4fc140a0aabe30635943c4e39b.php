        ...
        <div class="dela-presets-container-2" style="padding: 0px;">
            <div class="dela-preset-container">
                <form class="dela-preset-2-2" onclick="return false" method="get">
                    ...
                    <div class="fd-box">
                        <span class="fsl dfs spanchl" onclick="stype(1)">Mobile</span>
                        <span class="fsr dfs spanchr" onclick="stype(2)">E-mail</span>
                    ...
                    <div id="emailcodebox" style="display: none">
                        <div style="display: flex;">
                            <input style="flex: 1;" type="text" class="pml" name="email" id="emailcode" placeholder="email code" required="" autocomplete="emailcode" >
                            <span onclick="getCode()" class="getcode" style="display: block;height: 44px;line-height: 44px;padding: 0 20px;user-select: none;"><?php echo L('提交');?></span>
                        </div>
                    </div>
                    ...
                    <input type="password" class="pml" name="password" id="pass-2" placeholder="Password" required="">
                    <input type="password"  class="pml" name="repeat" placeholder="Repeat Password" oninput="this.setCustomValidity(this.value != document.getElementById('pass-'+2).value ? 'Passwords are not the same.' : '')" required="">
                    <div style="background: #212332;width: 100%;height: 44px;border-radius: 5px ;">
                        <input type="text" class="pml fl"  name="code" placeholder="Code"  class="code"  required=""   style="width: 70%;border-radius:  5px 0 0 5px;border: none" >
                        <input type="submit" class="pml fl fbg"  name="code" placeholder="Send"  class="send-btn"  required=""   style="width: 30%;border-radius: 0 5px 5px 0; text-align: center;background: #212332; border: none; color: #3db485;margin-bottom: 1.3em;border: 1px solid #212332;height: 23px;" >
                    </div>
                    <input type="submit" name="submit" class="fbg"  onclick="upreg()" value="<?php echo L('提交');?>">
                </form>
            </div>
        </div>
...
<script type="text/javascript">
    ...
    var emailACode
    function getStorage(){
        emailACode = localStorage.getItem('emailACode')
        if(emailACode){
            emailACode = JSON.parse(emailACode)
            console.log(emailACode, 64654654)
            if((emailACode.time + 300000) < +new Date()){
                localStorage.removeItem('emailACode')
                canGetCode = true
            }else{
                canGetCode = false
            }
    ...
    function getCode(){
        getStorage()
        if(!canGetCode) {
            layer.msg("<?php echo L('验证码5分钟有效，请稍后再试');?>");return false; 
        }
        var email = $("#email").val();
        ...
        $.post("<?php echo U('Login/getcodesss');?>",
            {email: email, type: 1},
            function(data){
                if(data.code == 1){
                    layer.msg(data.info);
                    data.time = +new Date()
                    localStorage.setItem('emailACode', JSON.stringify(data))
                }else{
    ...
    function upreg(){
        ...
        if (type == 2) {
            ...
            if(emailACode.msg != emailcode){
                layer.msg("<?php echo L('验证码错误');?>");return false;
            }
            // Verification Code Error
            if(emailACode.email != email){
                layer.msg("<?php echo L('邮箱不一致');?>");return false;
            }
        }
