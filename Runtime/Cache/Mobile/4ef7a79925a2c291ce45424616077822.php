...
<body>
	<div class="container-fluid " style="padding:0px;width:100vw;">
		...
		<?php if(empty($iframeid)): ?><div class="no_header">
		...
		<?php if(empty($iframeid)): ?><div class="contentbox">
		</else>
			<div class="contentbox" style="margin-top: 0px;"><?php endif; ?>




			<!-----------合约订单列表---------->
		    <div id="list_box" style="width:100%;background:#121420;padding: 0px 10px; margin-top:60px; display:block">
...
<script type="text/javascript">
    ...
    // helper to format ploss to 2 decimal places, return '--' if invalid
    function formatPloss(v){
        if(v === null || v === undefined || v === '') return '--';
        var n = parseFloat(v);
        if(isNaN(n)) return '--';
        return n.toFixed(2);
    }
    // helper to format prices to 2 decimal places, return '--' if invalid
    ...
    // helper to format timestamp to user's local time (MM-DD HH:mm format)
    function formatBuyTime(timestamp){
                ...
                if(data.data == '' || data.data == null){
                    ...
                    $.each(data.data,function(key,val){
                        html += '<div class="listbox">'+
                                ...
                                '<span class="fcc fe6im" style="font-size:16px;font-weight:500;">'+"<?php echo L('交易对');?>"+':</span>'+
                                ...
                                '</div>'+
                                '<div style="width:100%;height:30px;line-height:30px;">'+
                                '<span class="fe6im">'+formatPrice(val.buyprice)+'</span>'+
                                '</div>'+
                                '</div>'+
                                '<div style="width:25%;height:60px;float:left;">'+
                                '<div style="width:100%;height:30px;line-height:40px;">'+
                                '<span style="color:#cbcbcb;">Win/Lose</span>'+
                                '</div>'+
                                '<div style="width:100%;height:30px;line-height:30px;">'+
                                (val.status == 2 ? '<span class=\"fe6im\" style=\"color:' + (val.is_win == 1 ? '#0ecb81' : (val.is_win == 2 ? '#f5465c' : '#ffffff')) + '\">' + (val.is_win == 1 ? ('+' + formatPloss(val.ploss)) : (val.is_win == 2 ? ('-' + formatPloss(val.ploss)) : (val.ploss ? formatPloss(val.ploss) : '--'))) + '</span>' : '<span class=\"fe6im\">--</span>')+
                                '</div>'+
                                '</div>'+
                                '<div style="width:25%;height:60px;float:left;">'+
                                ...
                                '<span class="fe6im">'+formatBuyTime(val.buytime)+'</span>'+
                                '</div>'+
                                '</div>'+
                ...
                if(data.data == '' || data.data == null){
                    ...
                    $.each(data.data,function(key,val){
                        html += '<div class="listbox">'+
                                ...
                                '<div style="width:25%;height:60px;float:left;">'+
                                '<div style="width:100%;height:30px;line-height:40px;">'+
                                '<span class="f12" style="color:#6B7785;">'+"<?php echo L('投入金额');?>"+'</span>'+
                                '</div>'+
                                '<div style="width:100%;height:30px;line-height:30px;">'+
                                ...
                                '<div style="width:25%;height:60px;float:left;">'+
                                '<div style="width:100%;height:30px;line-height:40px;">'+
                                '<span class="f12"  style="color:#6B7785;">'+"<?php echo L('建仓单价');?>"+'</span>'+
                                '</div>'+
                                '<div style="width:100%;height:30px;line-height:30px;">'+
                                '<span class=" f12 fce5">'+formatPrice(val.buyprice)+'</span>'+
                                '</div>'+
                                '</div>'+
                                '<div style="width:25%;height:60px;float:left;">'+
                                '<div style="width:100%;height:30px;line-height:40px;">'+
                                '<span class="f12"  style="color:#6B7785;">Win/Lose</span>'+
                                '</div>'+
                                '<div style="width:100%;height:30px;line-height:30px;">'+
                                (val.status == 2 ? '<span class=\" f12 fce5\" style=\"color:' + (val.is_win == 1 ? '#0ecb81' : (val.is_win == 2 ? '#f5465c' : '#ffffff')) + '\">' + (val.is_win == 1 ? ('+' + formatPloss(val.ploss)) : (val.is_win == 2 ? ('-' + formatPloss(val.ploss)) : (val.ploss ? formatPloss(val.ploss) : '--'))) + '</span>' : '<span class=\" f12 fce5\">--</span>')+
                                '</div>'+
                                '</div>'+
                                '<div style="width:25%;height:60px;float:left;">'+
                                '<div style="width:100%;height:30px;line-height:40px;text-align:right;">'+
                                '<span class="f12"  style="color:#6B7785;">'+"<?php echo L('建仓时间');?>"+'</span>'+
                                '</div>'+
                                '<div style="width:100%;height:30px;line-height:30px;text-align:right;">'+
                                '<span class=" f12 fce5">'+formatBuyTime(val.buytime)+'</span>'+
                                '</div>'+
                                '</div>'+
                                '</div>'+
